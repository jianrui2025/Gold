...
class CallBackV2_MeanLineAndBollingerBands(CallBackV2Base):
    ...
    def __init__(self):
        ...
        self.output_file = "/home/jianrui/workspace/Gold/CallBack/log_400/{fund_code}"
        self.max_len = 400
...
class CallBackV2_MeanLineAndVolume(CallBackV2Base):
    # 使用 均线和成交量 的策略
    ...
    def __init__(self):
        ...
        self.HyperParam_dict = {
            "mean_long_day":[9,12,13,18,21,24,27,30,33,36,39,42,45,48,51,54,57,60,63],
            "mean_short_day":[3,6,9,12,13,18,21,24,27,30],
            "Volume_day":[4,6,8,10,12,14,16,18,20],
            "ShouYi":[0.005,0.01,0.015,0.02,0.025,0.03,0.035,0.04],
            "ZhiShun":[-0.005,-0.01,-0.015,-0.02,-0.025,-0.03,-0.035,-0.04],
        }
        self.output_file = "/home/jianrui/workspace/Gold/CallBack/log_400/{fund_code}"
    ...
    def downLoadData(self,**kwargs):
        if "fund_code" in kwargs:
            df_k = adata.fund.market.get_market_etf(fund_code=kwargs["fund_code"])
            df_k = df.to_dict(orient="records")
            df_min = adata.fund.market.get_market_etf_min(fund_code=kwargs["fund_code"])
            df_min = df_min.to_dict(orient="records")
            df_min = {i["trade_time"]:i for i in df_min}
            if self.max_len > 0:
                df = df[-self.max_len:]
            return {"k":df_k,"min":df_min}
        else:
            raise

    def run_onece(self,**kwrags):
        data = kwrags["data"]
        df_k = data["k"].to_dict(orient="records")
        df_min = data["min"].to_dict(orient="records")
        kwrags.pop("data")
        mean_long_day = kwrags["mean_long_day"]
        mean_short_day = kwrags["mean_short_day"]
        ...
        # 接下来就是一天的时间
        for index_day in range(max_day,len(df_k)):
            # 计算成交量的平均占比
            # TODO  



            current_data = df_k[index_day]
            if single == 0:
                price = float(current_data["low"])+0.002
                while price <= float(current_data["high"])-0.002:
                    mean_long_value = statistics.mean(
                        [float(i["close"])  for i in df_k[index_day-mean_long_day:index_day]] + [price]
                        )
                    mean_short_value = statistics.mean(
                        [float(i["close"])  for i in df_k[index_day-mean_short_day:index_day]] + [price]
                        )
